# Exact-25 adaptive causal STRIDER v32

This replaces the over-budget v30 mechanism diagnostic. V30 is excluded from
equal-budget claims. V32 uses exactly 25 new search episodes per task and is
...
V32 removes v31's automatic uniform `2.5x` bracket after a safe uniform `3x`
diagnostic. Native remains charged causal-reference evidence but is not eligible
for finalist confirmation. If no accelerated finalist clears the frozen gate,
the result is `no_acceleration_certified`, not a native-speed STRIDER selection.
...
1. Slot 1 is matched native `1x`; it must succeed `3/3` without incidents and
   is never a normal finalist.
...
5. After a safe uniform `3x` proposal, back off exactly one phase by one rung,
   ranked by successful current-search phase workload. This is recorded as a
   workload-derived robustness guard, not causal failure attribution. Otherwise
   promote one unfrozen phase by one rung using current-search bang-for-buck.
   Never change multiple phases except the preregistered uniform diagnostics.

Every serious proposal is evaluated on all three discovery seeds immediately.
A discovery-safe controller requires `3/3` and zero incidents. The two
accelerated finalists are frozen before confirmation outcomes; native cannot
consume either five-episode extension. Selection requires at least `7/8`, zero
incidents, then ranks successes, failure-aware throughput, and simplicity. If
neither accelerated finalist qualifies, seal `no_acceleration_certified`.
