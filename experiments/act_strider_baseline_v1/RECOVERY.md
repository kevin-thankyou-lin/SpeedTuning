...
- The first correctly pinned grouped workflow used Pick as its lead task.
  Pick completed first, which ended the group after Insertion had sealed 40 of
  its 50 final receipts. Insertion had no failure and its search plus 40 final
  receipts remain immutable.
- `act_strider_baseline_insertion_resume_l40.yaml` requires that exact partial
  state: matching identity and selection receipts, exactly 40 final receipts,
  and no result/completion marker. The benchmark runner verifies every cached
  seed and executes only the ten missing final seeds.
