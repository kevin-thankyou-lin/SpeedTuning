...
class Network(nn.Module):
    def __init__(
        ...
        # Visual observations are encoded into finite feature vectors before
        # reaching Rainbow, so the same MLP handles state-only and multimodal
        # observation stacks. ``use_state`` remains for checkpoint compatibility.
    ...
    def update_norm_stats(self, norm_stats: dict):
        with torch.no_grad():
            mean = torch.as_tensor(
                norm_stats['states_mean'], dtype=self.states_mean.dtype,
                device=self.states_mean.device,
            )
            std = torch.as_tensor(
                norm_stats['states_std'], dtype=self.states_std.dtype,
                device=self.states_std.device,
            ).clamp_min(1e-6)
            self.states_mean.copy_(mean)
            self.states_std.copy_(std)

    def dist(self, x: torch.Tensor) -> torch.Tensor:
        """Get distribution for atoms."""
        x = (x - self.states_mean) / self.states_std.clamp_min(1e-6)
        ...
        dist = F.softmax(q_atoms, dim=-1).clamp_min(1e-6)
