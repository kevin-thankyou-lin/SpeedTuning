#!/usr/bin/env python3
"""Build fresh outcome-blind stratified reset panels for STRIDER v27."""
...
SEEDS = {"pick": 27701, "tea": 27702, "insertion": 27703}
PANEL_IDS = {
    "pick": list(range(274000000, 274000008)),
    "tea": list(range(274000100, 274000108)),
    "insertion": list(range(274000200, 274000208)),
}
...
def stratified(task: str) -> np.ndarray:
    ...
    blocks = []
    for _ in range(2):
        block = np.empty((4, dimensions), dtype=np.float64)
        for column in range(dimensions):
            strata = rng.permutation(4)
            block[:, column] = (strata + rng.random(4)) / 4.0
        blocks.append(block)
...
def build(task: str) -> dict:
    ...
    template = None
    if task == "tea":
        template = np.asarray(sample_object_pose("tea_bag", 274099999), dtype=np.float64)
    poses = base.build_pose_vectors(task, positions, tea_template=template)
    return {
        "schema": "act-common-grid-strider-panel-v27",
        "task_label": task,
        "assumed_reset_prior": "independent_uniform_over_declared_position_bounds",
        "design": "two fresh randomized four-point Latin-hypercube blocks",
