...
Start with `info` and inspect `media/native.mp4`. Test schedules with `test A,B,C,D`; each unique schedule costs one episode and returns its phase-entry trace and MP4. Use visual causal reasoning to protect weak phases and raise phases with visible margin. You may stop early. Before terminating, call `select HASH` for one tested schedule that succeeded without a workspace violation. Optimize for likely randomized-pose generalization first and speed second; one-scene success is not a reliability claim. Do not access other experiments, seeds, private state, or prior schedules.
