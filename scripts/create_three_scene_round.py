...
Do not assign success probabilities. Choose two distinct schedules with safe `3/3` discovery results, then call `rank HASH1 HASH2` exactly once. The runner evaluates both on the same ten fresh randomized poses and automatically selects by: fewest safety violations, most successes, then lowest successful mean steps. Preserve twenty episodes for this ranking. Do not treat discovery success as a reliability estimate or access other experiments, seeds, private state, oracle phases, or prior schedules.
