...
The runner has three frozen discovery scenes and ten additional randomized ranking poses. Start with `info` and inspect all three native MP4s. The first accelerated challenger is preregistered: call `probe 2,2,2,2`. Use `probe A,B,C,D` for at most four further candidate schedules after reading that anchor's three outcomes. Every probe always runs all three discovery poses, including after a failure; compare the three outcomes before proposing another schedule. If the anchor is reliable, raise phases toward `3-4x`; if it is imperfect, repair from that working neighborhood rather than jumping immediately to the ceiling.

Keep native `1x` only as an external deployment fallback; it never occupies an accelerated finalist slot. Preserve the runner's six-episode causal-ladder reserve. Before ranking, always call `backoff HASH PHASE EVIDENCE` exactly once on the runner's `preferred_backoff_base_hash`: the fastest safe `3/3` schedule, or, when none is safe, the highest-success completed three-pose schedule with speed as tie-breaker. Attribute the phase from the three pose-aligned MP4s and telemetry, and give concise evidence. The runner holds every other phase fixed and tests one-rung and two-rung slowdowns of only that phase on all three scenes. It designates the fast base and the first safe causal backoff; if neither backoff is safe, it ranks the imperfect base alone. Do not invent another ladder after reading outcomes.
