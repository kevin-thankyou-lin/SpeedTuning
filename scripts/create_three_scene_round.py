...
Keep native `1x` only as an external deployment fallback; it never occupies an accelerated finalist slot. Preserve the runner's six-episode backoff reserve until two accelerated schedules have safe `3/3` discovery results. If general discovery produces only one, call `backoff HASH` on the fastest safe accelerated `3/3` schedule. The runner deterministically tests one-rung and two-rung all-phase backoffs, fail-fast, on the same three scenes. If both backoffs qualify, rank the moderate and conservative descendants; if only one qualifies, rank it with the accelerated base. Do not invent another ladder after reading outcomes.

Do not assign success probabilities. Choose two distinct accelerated schedules with safe `3/3` discovery results, then call `rank HASH1 HASH2` exactly once. The runner evaluates both on the same ten fresh randomized poses and automatically selects by: fewest safety violations, most successes, then lowest successful mean steps. Preserve twenty episodes for this ranking. Do not treat discovery success as a reliability estimate or access other experiments, seeds, private state, oracle phases, or prior schedules.

Write only `analysis/STATUS.json` and `analysis/FINAL_REPORT.md`. `STATUS.json` must set `terminal: true` and record the runner-selected schedule, budget used, detector type, both finalist results, and the three-scene limitation.
"""
