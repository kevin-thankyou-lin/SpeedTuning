...
The runner has three frozen discovery scenes and ten additional randomized ranking poses. Start with `info` and inspect the three native MP4s. Use `probe A,B,C,D` for at most nine distinct schedules on scene A. Promote at most five successful probes with `screen HASH`; screening runs scene B and, only after success, scene C. Use `refine A,B,C,D` for nearby causal repairs or variants; it tests the three discovery scenes fail-fast within the eight-episode refinement pool.

Keep native `1x` only as an external deployment fallback; it never occupies an accelerated finalist slot. Preserve the runner's six-episode backoff reserve. Before ranking, always call `backoff HASH` exactly once on the fastest safe accelerated `3/3` schedule, even if discovery already found multiple accelerated survivors. The runner deterministically tests one-rung and two-rung all-phase backoffs, fail-fast, on the same three scenes. It designates the moderate and conservative descendants as finalists when both pass; if only one passes, it designates that descendant and the accelerated base. Do not invent another ladder after reading outcomes.

Do not assign success probabilities. Call `rank HASH1 HASH2` exactly once with the two runner-designated ladder finalists. The runner evaluates both on the same ten fresh randomized poses. A schedule is provisionally qualified only with zero safety violations and at least `9/10`; among qualifiers it selects by successes and successful mean steps. If neither qualifies, deployment falls back to native while the least-bad accelerated schedule is still frozen for a descriptive 100-state benchmark. Preserve twenty episodes for ranking. Do not treat discovery success or `9/10` as certification, and do not access other experiments, seeds, private state, oracle phases, or prior schedules.

Write only `analysis/STATUS.json` and `analysis/FINAL_REPORT.md`. `STATUS.json` must set `terminal: true` and record accelerated qualification, deployment schedule, benchmark schedule, budget used, detector type, both finalist results, and the three-scene limitation.
"""
