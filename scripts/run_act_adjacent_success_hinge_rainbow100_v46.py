#!/usr/bin/env python3
"""Run the fresh Adjacent-success-hinge Rainbow-100 replication study V46."""
...
SEARCH_METHOD = "adjacent_success_hinge_rainbow100"
FINAL_METHODS = ("adjacent_success_hinge",)
...
def expand_bank(spec: dict) -> list[int]:
    ...
    if count <= 0:
        raise RuntimeError("v46 seed range count must be positive")
...
def validate_banks(banks: dict) -> None:
    if banks.get("schema") != "act-adjacent-success-hinge-rainbow100-banks-v46":
        raise RuntimeError("v46 bank schema differs")
    all_values: list[int] = []
    ...
    for task in TASKS:
        ...
        for name, count in expected.items():
            if len(values[name]) != count:
                raise RuntimeError(f"v46 {task}/{name} count differs")
        flattened = [seed for bank in values.values() for seed in bank]
        if len(flattened) != len(set(flattened)) or min(flattened) < 430000000:
            raise RuntimeError(f"v46 {task} banks overlap or are not fresh")
        all_values.extend(flattened)
    if len(all_values) != len(set(all_values)):
        raise RuntimeError("v46 cross-task banks overlap")
...
def optimizer_diagnostics(records: list[dict]) -> dict:
    ...
    return {
        ...
        "mean_adjacent_success_loss": weighted(
            "mean_adjacent_success_loss"
        ),
        "mean_adjacent_success_active_fraction": weighted(
            "mean_adjacent_success_active_fraction"
        ),
        "adjacent_success_comparisons": sum(
            int(item["adjacent_success_comparisons"]) for item in values
        ),
        "successful_episodes_seen": max(
            (int(item["adjacent_success_successful_episodes_seen"])
             for item in values),
            default=0,
        ),
        "accepted_nonregressive_episodes": max(
            (int(item["adjacent_success_accepted_episodes"])
             for item in values),
            default=0,
        ),
        "rejected_regressive_episodes": max(
            (int(item["adjacent_success_rejected_regression"])
             for item in values),
            default=0,
        ),
    }
...
class V46Runtime:
    ...
    def __init__(self, frontier: ACTFrontierRuntime, task_label: str, device: str,
        ...
        self.prereg["training"].update({
            "algorithm": "adjacent_success_hinge_dqn",
            "lql_trajectory_length": 0,
            "lql_lambda_lb": 0.0,
            "lql_lambda_ub": 0.0,
            "adjacent_success_trajectory_length": (
                ADJACENT_SUCCESS_TRAJECTORY_LENGTH
            ),
            "adjacent_success_lambda": ADJACENT_SUCCESS_LAMBDA,
            "adjacent_success_hinge": "mean_relu_q_slow_minus_q_chosen",
            "adjacent_success_margin": 0.0,
            "adjacent_success_norm": "categorical_support_width",
            "adjacent_action": "exactly_one_speed_rung_slower",
            "trajectory_label": (
                "safe_terminal_success_and_nonregressive_task_reward_at_"
                "decision_endpoints"
            ),
            "failed_trajectory_constraints": False,
        })
    ...
    def validate_search_record(self, record: dict) -> None:
        if record.get("physics_error") is not None:
            raise RuntimeError("v46 halted immediately on a Rainbow physics error")
        if record["environment_spec"].get("randomize_object_pose") is not True:
            raise RuntimeError("v46 training did not use a randomized reset")

    def environment(self, seed: int, *, training=False):
        seed = int(seed)
        if training and seed not in self.training_seeds:
            raise RuntimeError("v46 training attempted an unregistered reset")
        if not training and seed not in self.probe_seeds | self.final_seeds:
            raise RuntimeError("v46 evaluation attempted an unregistered reset")
        ...
        env._environment_metadata["v46_reset_regime"] = "randomized_object_pose"
        return env
...
def build_checkpoint_policy(training_root: Path, episode: int, config: dict) -> tuple[Path, str]:
    ...
    if len(state_files) != TRAINING_EPISODES:
        raise RuntimeError("v46 checkpoint conversion requires sealed training")
    ...
    if file_sha256(resume) != record["resume_checkpoint_sha256"]:
        raise RuntimeError(f"v46 episode-{episode} resume checkpoint differs")
...
def controller(kind: str, *, checkpoint_sha256: str | None = None,
    ...
    if kind == "adjacent_success_hinge":
        value = {
            "type": "phase_conditioned_adjacent_success_hinge_rainbow100",
...
def run_probe(runtime: V46Runtime, output: Path, search_identity: str, task: str,
              stage: str, seeds: list[int], controller_value: dict, policy) -> tuple[dict, list[dict]]:
    identity = {
        "schema": "act-adjacent-success-hinge-rainbow100-probe-identity-v46",
    ...
    for seed in seeds[len(records):]:
        ...
        if record.get("physics_error") is not None:
            raise RuntimeError(f"v46 halted on {stage} physics error")
    result = {
        "schema": "act-adjacent-success-hinge-rainbow100-probe-result-v46",
        "task_label": task,
        "stage": stage,
        "controller": controller_value,
    ...
    immutable_or_verify(output / "COMPLETE.json", {
        "schema": "act-adjacent-success-hinge-rainbow100-probe-completion-v46",
        "episodes": len(records),
...
def run_search(runtime: V46Runtime, root: Path, task: str, spec: dict,
    ...
    identity = {
        **runtime.frontier.identity(),
        "schema": "act-adjacent-success-hinge-rainbow100-search-identity-v46",
        "task_label": task,
        ...
        "paired_baselines": {
            "plain_rainbow_v43_commit": (
                "989614daf538bebbbd31fbafea544f6911321683"
            ),
            "paper_lql_v45_commit": (
                "39ddd18dbc0fbf78ec379ca6aec259d13e28f07f"
            ),
            "same_training_probe_and_final_seed_banks": True,
            "baseline_rollouts_reexecuted": 0,
        },
        "adjacent_success_hinge": {
            "trajectory_length": ADJACENT_SUCCESS_TRAJECTORY_LENGTH,
            "lambda": ADJACENT_SUCCESS_LAMBDA,
            "loss": "zero_margin_l1_hinge",
            "comparison": "chosen_speed_vs_adjacent_slower_speed",
            "eligibility": "safe_success_and_nonregressive_task_reward_trace",
            "failure_constraints": False,
            "all_slower_speed_constraints": False,
        },
    ...
    if (output / "SEARCH_COMPLETE.json").exists():
        ...
        if complete["terminal_policy_sha256"] != file_sha256(
            ...
            raise RuntimeError("v46 completed terminal policy differs")
        return

    training_identity = canonical_sha256({
        "search_identity_sha256": identity["identity_sha256"],
        "stage": "adjacent_success_hinge_training",
    })
    ...
    if len(records) != TRAINING_EPISODES:
        raise RuntimeError("v46 training did not seal exactly 100 episodes")
    training_incidents = incidents(records)
    if training_incidents["physics_errors"]:
        raise RuntimeError("v46 training contains a physics error")
    ...
    for episode in CHECKPOINT_EPISODES:
        ...
        value = controller(
            "adjacent_success_hinge", checkpoint_sha256=checkpoint_sha, episode=episode
        )
    ...
    result = {
        "schema": "act-adjacent-success-hinge-rainbow100-search-result-v46",
        "task_label": task,
    ...
    if result["prefinal_scientific_rollouts"] != PREFINAL_ROLLOUTS:
        raise RuntimeError("v46 prefinal accounting differs")
    immutable_or_verify(output / "SEARCH_RESULT.json", result)
    immutable_or_verify(output / "SEARCH_COMPLETE.json", {
        "schema": "act-adjacent-success-hinge-rainbow100-search-completion-v46",
        "task_label": task,
        "training_episodes": TRAINING_EPISODES,
...
def require_all_search(root: Path) -> None:
    for task in TASKS:
        ...
        if int(complete.get("training_episodes", -1)) != TRAINING_EPISODES:
            raise RuntimeError(f"v46 search incomplete: {task}")
...
def run_final(runtime: V46Runtime, root: Path, task: str, method: str,
    ...
    identity = {
        **runtime.frontier.identity(),
        "schema": "act-adjacent-success-hinge-rainbow100-final-identity-v46",
        "task_label": task,
        "method": method,
        "controller": controller_value,
    ...
    for seed in seeds[len(records):]:
        ...
        if record.get("physics_error") is not None:
            raise RuntimeError(f"v46 halted on final physics error: {task}/{method}")
    result = {
        "schema": "act-adjacent-success-hinge-rainbow100-final-result-v46",
        "task_label": task,
        "method": method,
        "controller": controller_value,
        "episodes": len(records),
    ...
    immutable_or_verify(output / "RESULT.json", result)
    immutable_or_verify(output / "COMPLETE.json", {
        "schema": "act-adjacent-success-hinge-rainbow100-final-completion-v46",
        "episodes": len(records),
        "result_sha256": file_sha256(output / "RESULT.json"),
...
def main() -> int:
    ...
    if git_head() != args.implementation_commit:
        raise RuntimeError("v46 checked-out source differs from implementation commit")
    ...
    runtime = V46Runtime(frontier, args.task_label, args.device, registered)
    root = args.root.resolve()
    if args.stage == "search":
        if args.method != SEARCH_METHOD:
            raise ValueError("v46 search stage requires adjacent_success_hinge_rainbow100")
        run_search(
            runtime, root, args.task_label, spec, args.contract, args.banks
        )
    else:
        if args.method not in FINAL_METHODS:
            raise ValueError("v46 final stage requires a registered final method")
        run_final(
