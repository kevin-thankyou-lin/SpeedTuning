...
class Ledger:
    ...
    def one(self, schedule, seed: int) -> dict:
        ...
        if path.exists():
            ...
            if int(record.get("seed", -1)) != seed:
                raise RuntimeError("cached v32 seed mismatch")
            if list(map(float, record.get("schedule", ()))) != schedule:
                raise RuntimeError("cached v32 schedule mismatch")
            return record
        if self.used() >= SEARCH_BUDGET:
            raise RuntimeError("v32 exact-25 search budget exhausted")
        record = self.runtime.rollout(
            schedule, seed, record_attribution_telemetry=True
        )
        if list(map(float, record.get("schedule", ()))) != schedule:
            raise RuntimeError("runtime returned a different v32 schedule")
    ...
    def discovery_report(self, schedule, role: str) -> tuple[dict, list[dict]]:
        ...
        if report["summary"]["physics_errors"]:
            raise RuntimeError(f"physics error in v32 discovery schedule {schedule}")
        write_json(
    ...
    def confirmation_report(self, report: dict) -> tuple[dict, list[dict]]:
        ...
        if value["summary"]["physics_errors"]:
            raise RuntimeError(f"physics error in v32 finalist {report['schedule']}")
        write_json(
...
def best_safe(reports: list[dict]) -> dict:
    ...
    if not candidates:
        raise RuntimeError("v32 has no safe discovery controller")
    return max(
        candidates,
        key=lambda report: (
...
def ranked_workload_guards(records, schedule, frozen: set[str]):
    """Back off one phase of a safe ceiling using only current-search telemetry."""
    usable = [record for record in records if successful(record)]
    if not usable:
        return []
    work = {
    ...
    candidates = []
    for phase, speed in zip(PHASES, schedule):
        if phase in frozen or speed == GRID[0]:
            continue
        guarded = v30.adjacent_speed(speed, -1)
        score = work[phase] * (1.0 / guarded - 1.0 / speed)
        proposal = list(schedule)
        proposal[PHASES.index(phase)] = guarded
        candidates.append((score, -PHASES.index(phase), phase, proposal, work[phase]))
    return sorted(candidates, reverse=True)
...
def next_safe_proposal(
    ...
    ceiling = next(
        (
            ...
            if report["schedule"] == [3.0] * len(PHASES) and safe(report)
        ),
        None,
    )
    if ceiling is not None:
        ceiling_records = records_by_hash[ceiling["schedule_sha256"]]
        for _, _, phase, schedule, workload in ranked_workload_guards(
            ceiling_records, ceiling["schedule"], frozen
        ):
            if schedule_sha256(schedule) not in tried:
                return schedule, {
                    "operation": "one_rung_workload_guard",
                    "source_schedule": ceiling["schedule"],
                    "proposed_schedule": schedule,
                    "phase": phase,
                    "evidence": {
                        "method": "current_search_successful_phase_workload",
                        "mean_phase_workload": workload,
                    },
                }
...
def run_search(ledger: Ledger, task: str) -> dict:
    ...
    if not safe(native):
        raise RuntimeError("v32 native reference is not safe 3/3")
    ...
    while len(reports) < DISCOVERY_SCHEDULES:
        ...
        if proposed is None or schedule_sha256(proposed) in tried:
            raise RuntimeError("v32 could not fill five unique discovery slots")
        last, last_records = evaluate(proposed, "causal_frontier")

    accelerated = [
        report
        for report in reports
        if any(float(speed) > 1.0 for speed in report["schedule"])
    ]
    if len(accelerated) < 2:
        raise RuntimeError("v32 requires two accelerated discovery candidates")
    ranked = sorted(
        accelerated,
        key=lambda report: (
            safe(report),
    ...
    if ledger.used() != SEARCH_BUDGET:
        raise RuntimeError(f"v32 search used {ledger.used()}, expected exactly 25")
    eligible = [
        value
    ...
    return {
        "schema": "act-common-grid-strider-causal25-selection-v32",
        "task_label": task,
        ...
        "selection_status": (
            "accelerated_schedule_certified"
            if selected is not None
            else "no_acceleration_certified"
        ),
        "selected_schedule": selected["schedule"] if selected is not None else None,
        "selected_schedule_sha256": (
            selected["schedule_sha256"] if selected is not None else None
        ),
...
def main() -> int:
    ...
    if len(all_seeds) != len(set(all_seeds)):
        raise RuntimeError("v32 registered banks overlap")
    ...
    identity = {
        **runtime.identity(),
        "schema": "act-common-grid-strider-causal25-identity-v32",
        "task_label": args.task_label,
    ...
    if identity_path.exists() and json.loads(identity_path.read_text()) != identity:
        raise RuntimeError("v32 identity differs on resume")
    ...
    if complete_path.exists():
        ...
        if complete["identity_sha256"] != file_sha256(identity_path):
            raise RuntimeError("v32 completed identity hash mismatch")
        if complete["selection_sha256"] != file_sha256(selection_path):
            raise RuntimeError("v32 completed selection hash mismatch")
    ...
    if selection_path.exists() and json.loads(selection_path.read_text()) != selection:
        raise RuntimeError("sealed v32 selection changed on resume")
