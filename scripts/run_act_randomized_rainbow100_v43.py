#!/usr/bin/env python3
"""Run the fresh randomized-reset Rainbow-100 replication study V43."""
...
SEARCH_METHOD = "randomized_rainbow100"
FINAL_METHODS = ("native_1x", "uniform_2x", "rainbow")
TRAINING_EPISODES = 100
CHECKPOINT_EPISODES = tuple(range(10, TRAINING_EPISODES + 1, 10))
PROBE_EPISODES = 3
FINAL_EPISODES = 50
PREFINAL_ROLLOUTS = (
    TRAINING_EPISODES + PROBE_EPISODES + len(CHECKPOINT_EPISODES) * PROBE_EPISODES
)
...
def expand_bank(spec: dict) -> list[int]:
    ...
    if count <= 0:
        raise RuntimeError("v43 seed range count must be positive")
...
def validate_banks(banks: dict) -> None:
    if banks.get("schema") != "act-randomized-rainbow100-banks-v43":
        raise RuntimeError("v43 bank schema differs")
    all_values: list[int] = []
    ...
    for task in TASKS:
        ...
        for name, count in expected.items():
            if len(values[name]) != count:
                raise RuntimeError(f"v43 {task}/{name} count differs")
        flattened = [seed for bank in values.values() for seed in bank]
        if len(flattened) != len(set(flattened)) or min(flattened) < 430000000:
            raise RuntimeError(f"v43 {task} banks overlap or are not fresh")
        all_values.extend(flattened)
    if len(all_values) != len(set(all_values)):
        raise RuntimeError("v43 cross-task banks overlap")
...
class V43Runtime:
    ...
    def validate_search_record(self, record: dict) -> None:
        if record.get("physics_error") is not None:
            raise RuntimeError("v43 halted immediately on a Rainbow physics error")
        if record["environment_spec"].get("randomize_object_pose") is not True:
            raise RuntimeError("v43 training did not use a randomized reset")

    def environment(self, seed: int, *, training=False):
        seed = int(seed)
        if training and seed not in self.training_seeds:
            raise RuntimeError("v43 training attempted an unregistered reset")
        if not training and seed not in self.probe_seeds | self.final_seeds:
            raise RuntimeError("v43 evaluation attempted an unregistered reset")
        ...
        env._environment_metadata["v43_reset_regime"] = "randomized_object_pose"
        return env
...
def build_checkpoint_policy(training_root: Path, episode: int, config: dict) -> tuple[Path, str]:
    ...
    if len(state_files) != TRAINING_EPISODES:
        raise RuntimeError("v43 checkpoint conversion requires sealed training")
    ...
    if file_sha256(resume) != record["resume_checkpoint_sha256"]:
        raise RuntimeError(f"v43 episode-{episode} resume checkpoint differs")
...
def controller(kind: str, *, checkpoint_sha256: str | None = None,
    ...
    if kind == "native_1x":
        value = {"type": "fixed_speed", "speed": 1.0}
    elif kind == "uniform_2x":
        value = {"type": "fixed_speed", "speed": 2.0}
    elif kind == "rainbow":
        value = {
            "type": "phase_conditioned_randomized_rainbow100",
...
def run_probe(runtime: V43Runtime, output: Path, search_identity: str, task: str,
              stage: str, seeds: list[int], controller_value: dict, policy) -> tuple[dict, list[dict]]:
    identity = {
        "schema": "act-randomized-rainbow100-probe-identity-v43",
    ...
    for seed in seeds[len(records):]:
        ...
        if record.get("physics_error") is not None:
            raise RuntimeError(f"v43 halted on {stage} physics error")
    result = {
        "schema": "act-randomized-rainbow100-probe-result-v43",
        "task_label": task,
        "stage": stage,
        "controller": controller_value,
    ...
    immutable_or_verify(output / "COMPLETE.json", {
        "schema": "act-randomized-rainbow100-probe-completion-v43",
        "episodes": len(records),
...
def run_search(runtime: V43Runtime, root: Path, task: str, spec: dict,
    ...
    identity = {
        **runtime.frontier.identity(),
        "schema": "act-randomized-rainbow100-search-identity-v43",
        "task_label": task,
    ...
    if (output / "SEARCH_COMPLETE.json").exists():
        ...
        if complete["terminal_policy_sha256"] != file_sha256(
            ...
            raise RuntimeError("v43 completed terminal policy differs")
        return

    training_identity = canonical_sha256({
        "search_identity_sha256": identity["identity_sha256"],
        "stage": "randomized_rainbow_training",
    })
    ...
    if len(records) != TRAINING_EPISODES:
        raise RuntimeError("v43 training did not seal exactly 100 episodes")
    training_incidents = incidents(records)
    if training_incidents["physics_errors"]:
        raise RuntimeError("v43 training contains a physics error")

    native_value = controller("native_1x")
    native_result, native_records = run_probe(
        runtime, output / "checkpoint_probes" / "native_1x",
        identity["identity_sha256"], task, "checkpoint_probe_native_1x",
        banks["probe"], native_value, FixedSpeedPolicy(1.0),
    )
    checkpoints = {}
    ...
    for episode in CHECKPOINT_EPISODES:
        ...
        value = controller(
            "rainbow", checkpoint_sha256=checkpoint_sha, episode=episode
        )
        ...
        checkpoints[str(episode)] = {
            ...
            "probe": result,
            "paired_vs_native_1x": v37.paired_receipt(episode_records, native_records),
        }
        probe_records.extend(episode_records)

    all_records = [*records, *native_records, *probe_records]
    totals = incidents(all_records)
    terminal_path = training_root / "terminal_policy.pt"
    result = {
        "schema": "act-randomized-rainbow100-search-result-v43",
        "task_label": task,
        ...
        "rollout_split": {
            ...
            "shared_native_probe": len(native_records),
    ...
    if result["prefinal_scientific_rollouts"] != PREFINAL_ROLLOUTS:
        raise RuntimeError("v43 prefinal accounting differs")
    immutable_or_verify(output / "SEARCH_RESULT.json", result)
    immutable_or_verify(output / "SEARCH_COMPLETE.json", {
        "schema": "act-randomized-rainbow100-search-completion-v43",
        "task_label": task,
        "training_episodes": TRAINING_EPISODES,
...
def require_all_search(root: Path) -> None:
    for task in TASKS:
        ...
        if int(complete.get("training_episodes", -1)) != TRAINING_EPISODES:
            raise RuntimeError(f"v43 search incomplete: {task}")
...
def method_policy(root: Path, task: str, method: str, device: str):
    ...
    if method == "native_1x":
        ...
        if file_sha256(checkpoint) != search["terminal_policy_sha256"]:
            raise RuntimeError("v43 deployment checkpoint differs")
        value = controller(
...
def run_final(runtime: V43Runtime, root: Path, task: str, method: str,
    ...
    identity = {
        **runtime.frontier.identity(),
        "schema": "act-randomized-rainbow100-final-identity-v43",
        "task_label": task,
        "method": method,
        "controller": controller_value,
    ...
    for seed in seeds[len(records):]:
        ...
        if record.get("physics_error") is not None:
            raise RuntimeError(f"v43 halted on final physics error: {task}/{method}")
    result = {
        "schema": "act-randomized-rainbow100-final-result-v43",
        "task_label": task,
        "method": method,
        "controller": controller_value,
        "episodes": len(records),
    ...
    immutable_or_verify(output / "RESULT.json", result)
    immutable_or_verify(output / "COMPLETE.json", {
        "schema": "act-randomized-rainbow100-final-completion-v43",
        "episodes": len(records),
        "result_sha256": file_sha256(output / "RESULT.json"),
...
def main() -> int:
    ...
    if git_head() != args.implementation_commit:
        raise RuntimeError("v43 checked-out source differs from implementation commit")
    ...
    runtime = V43Runtime(frontier, args.task_label, args.device, registered)
    root = args.root.resolve()
    if args.stage == "search":
        if args.method != SEARCH_METHOD:
            raise ValueError("v43 search stage requires randomized_rainbow100")
        run_search(
            runtime, root, args.task_label, spec, args.contract, args.banks
        )
    else:
        if args.method not in FINAL_METHODS:
            raise ValueError("v43 final stage requires a registered final method")
        run_final(
