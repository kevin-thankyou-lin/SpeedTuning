...
from scripts import run_act_common_grid_strider_causal25_v32 as module
...
def test_exact_25_budget_includes_native_and_confirmation(tmp_path):
    ...
    assert all(item["schedule"] != [1.0] * 4 for item in selection["finalists"])
...
def test_safe_ceiling_generates_phasewise_guards_not_uniform_bracket(tmp_path):
    class Runtime:
        def rollout(self, schedule, seed, *, record_attribution_telemetry=False):
            return record(seed, list(schedule), True, steps=100)

    ledger = module.Ledger(Runtime(), tmp_path, [0, 1, 2], [10, 11, 12, 13, 14])
    selection = module.run_search(ledger, "pick")
    schedules = [item["schedule"] for item in selection["discovery_reports"]]
    assert schedules[:3] == [[1.0] * 4, [2.0] * 4, [3.0] * 4]
    assert [2.5] * 4 not in schedules
    assert all(
        sum(float(speed) == 2.5 for speed in schedule) == 1
        and sum(float(speed) == 3.0 for speed in schedule) == 3
        for schedule in schedules[3:]
    )
    guards = [
        item
        for item in selection["update_receipts"]
        if item["operation"] == "one_rung_workload_guard"
    ]
    assert len(guards) == 2
    assert all(item["schedule"] != [1.0] * 4 for item in selection["finalists"])
    assert selection["search_scientific_rollouts"] == ledger.used() == 25
...
def test_native_is_reference_only_and_no_acceleration_can_be_certified(tmp_path):
    class Runtime:
        def rollout(self, schedule, seed, *, record_attribution_telemetry=False):
            schedule = list(schedule)
            return record(
    ...
    assert all(item["schedule"] != [1.0] * 4 for item in selection["finalists"])
    assert selection["selection_status"] == "no_acceleration_certified"
    assert selection["selected_schedule"] is None
    assert selection["selected_schedule_sha256"] is None
    assert selection["search_scientific_rollouts"] == ledger.used() == 25


def test_banks_are_fresh_disjoint_and_final_unopened():
    banks = json.loads(
        (module.REPO_ROOT / "experiments/act_common_grid_strider_causal25_v32/BANKS.json").read_text()
    )
    seeds = []
